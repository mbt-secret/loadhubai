...
function isVisibleWhatsappGroup(group) {
  ...
  const whatsappId = String(group.whatsappId ?? '');

  if (!name || /[\r\n\u202A-\u202E]/.test(rawName)) return false;
  // Exclude pseudo-surse care nu sunt grupuri WhatsApp (ex: GeoTrans).
  if (whatsappId.startsWith('source:')) return false;
...
app.listen(port, host, () => {
  console.log(`TIRGo server: http://${host}:${port}`);
});
